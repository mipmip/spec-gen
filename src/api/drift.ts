...
import { DEFAULT_DRIFT_MAX_FILES, DEFAULT_ANTHROPIC_MODEL, DEFAULT_OPENAI_MODEL, DEFAULT_GEMINI_MODEL, DEFAULT_OPENAI_COMPAT_MODEL, DEFAULT_BEDROCK_MODEL, SPEC_GEN_DIR, SPEC_GEN_ANALYSIS_SUBDIR, SPEC_GEN_LOGS_SUBDIR, OPENSPEC_DIR, OPENSPEC_SPECS_SUBDIR, ARTIFACT_REPO_STRUCTURE } from '../constants.js';
...
export async function specGenDrift(options: DriftApiOptions = {}): Promise<DriftResult> {
  ...
  if (llmEnhanced) {
    ...
    if (!anthropicKey && !openaiKey && !openaiCompatKey && !geminiKey && !bedrockToken) {
      throw new Error('No LLM API key found. LLM-enhanced drift requires ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, OPENAI_COMPAT_API_KEY, or AWS_BEARER_TOKEN_BEDROCK.');
    }
